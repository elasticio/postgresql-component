const logger = require('@elastic.io/component-logger')();

const pgp = require('pg-promise')({
  connect(client) {
    const cp = client.connectionParameters;
    logger.info('Connected to database:', cp.database);
  },
  disconnect(client) {
    const cp = client.connectionParameters;
    logger.info('Disconnecting from database:', cp.database);
  },
  transact(e) {
    if (e.ctx.finish) {
      if (e.ctx.success) {
        logger.info('Transaction was executed successfully');
      } else {
        e.ctx.result.getErrors().forEach((err) => {
          logger.info('Transaction execution error:\n', err.stack);
        });
        logger.info('Transaction was executed unsuccessfully');
        throw (e);
      }
    } else {
      logger.info('Transaction is starting... Start Time:', e.ctx.start);
    }
  },
});

/**
 * This is PG-promise client
 *
 * @param cfg configuration that contains conString parameter
 */
module.exports.getDb = function getDb(cfg) {
  const {
    host,
    port,
    database,
    user,
    password,
    conString,
  } = cfg;
  const db = pgp(conString || {
    host,
    port,
    database,
    user,
    password,
  });
  return db;
};

module.exports.detachDb = function detachDb() {
  pgp.end();
};
