...
function processAction(msg, cfg) {
  ...
  self.logger.info('Going to execute query sqlQuery...');
  ...
  pool.connect((err, client, done) => {
    ...
    if (err) {
      self.logger.error('Error occurred while fetching client from pool!');
    ...
    loop(nextBatchPromise(cursor, self, isBatch)).fail((err) => {
      this.logger.error('Error occurred when executing the query!');
      done(client);
      self.emit('error', err);
      return self.emit('end');
    }).done(() => {
