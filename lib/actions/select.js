...
function processAction(msg, cfg) {
  ...
  pg.connect(conString, (err, client, done) => {
    ...
    function loop(promise) {
      return promise.then(result => (result ? loop(nextBatchPromise(cursor, self, isBatch)) : undefined));
    }
...
function nextBatchPromise(cursor, self, sendAsArray) {
  ...
  cursor.read(ROWS_BATCH_SIZE, (err, rows) => {
    ...
    if (rows.length < ROWS_BATCH_SIZE) {
      console.log('All rows were fetched, last batch size was size=%s', rows.length);
      return defer.resolve(false);
    }
    // Continue loop
    defer.resolve(true);
