...
function processAction(msg, cfg) {
  ...
  this.logger.info('Going to merge SQL template with msg sql="%s" body=%j', sql, msg.body);
  const q = utils.prepareStatement(sql, msg, script);
  this.logger.info('Going to execute query sqlQuery=%s values=%j', q.text, q.values);
...
function nextBatchPromise(cursor, self, sendAsArray) {
  const defer = Q.defer();
  this.logger.info('Fetching next batch of rows size=%s', ROWS_BATCH_SIZE);
  cursor.read(ROWS_BATCH_SIZE, (err, rows) => {
    ...
    if (!rows.length) {
      ...
      this.logger.info('All rows fetched, last batch size was size=0');
    ...
    if (rows.length < ROWS_BATCH_SIZE) {
      this.logger.info('All rows were fetched, last batch size was size=%s', rows.length);
      return defer.resolve(false);
    }
    // Continue loop
    defer.resolve(true);
