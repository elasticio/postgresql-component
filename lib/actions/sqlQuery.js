...
function processAction(msg, cfg) {
  ...
  this.logger.info('SQL parsing finished');
  this.logger.info('Going to execute query');
  ...
  pool.connect((err, client, done) => {
    ...
    if (err) {
      this.logger.error('Error fetching client from pool occurred');
    ...
    loop(nextBatchPromise(cursor, self)).fail((err) => {
      this.logger.error('Error when executing the query occurred');
      done(client);
      self.emit('error', err);
      return self.emit('end');
    }).done(() => {
...
function nextBatchPromise(cursor, self) {
  const defer = Q.defer();
  self.logger.info('Fetching next 1000 rows');
...
function getMetaModel(cfg, cb) {
  ...
  if (sql && sql.length > 0) {
    try {
      const result = utils.parseSQL(sql);
      this.logger.info('Parsed SQL successfully');
      ...
      this.logger.error('Exception happened');
      return cb(error);
    }
  }
  cb(null, {
