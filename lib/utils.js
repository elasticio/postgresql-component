/* eslint-disable no-param-reassign,max-len,prefer-destructuring,no-plusplus,no-undef,no-shadow,no-unused-vars,no-new */
...
function transformFields(field) {
  ...
  if (field.field instanceof nodes.LiteralValue && field.field.nested === false && field.field.value) {
    ...
    this.logger.error(`Can not transform field ${field}`);
 ...
 */
function prepareQuery(logger, sql, body) {
  logger.info('Received SQL query = %s', sql);
  ...
  let query;
  if (vars) {
    logger.info('Found following prepared variable:type pairs=%j', vars);
    const values = [];
    let text = sql;
    // eslint-disable-next-line
  ...
  logger.info('Preparing statement created. Resulting query=%s', JSON.stringify(query));
  return query;
}
 ...
 */
function prepareQueries(logger, sql, body) {
  logger.info('Received SQL queries: %s', sql);
  const queries = sql
  ...
  const preparedSql = [];
  queries.forEach((query) => preparedSql.push(prepareQuery(logger, query, body)));
  return preparedSql;
}
 ...
 */
module.exports.executeQuery = async function executeQuery(logger, sql, values, cfg) {
  let retriesLeft = cfg.retriesLeft || 0;
  logger.info('Retries left:', retriesLeft);
  const queries = prepareQueries(logger, sql, values);
  ...
  return db.tx((t) => {
    ...
    logger.info(`Query successfully executed, Data ${JSON.stringify(data)}`);
    clientPgPromise.detachDb(logger);
    return data;
  }).catch((error) => {
    logger.info('Error:', error.message || error);
    if (error.message === 'deadlock detected' && retriesLeft) {
      new Promise((resolve, reject) => setTimeout(() => {
        logger.info('Trying to re-run expression in 1 second');
        cfg.retriesLeft = --retriesLeft;
        clientPgPromise.detachDb(logger);
        this.executeQuery(sql, values, cfg);
      }, 1000),
      ((error, response, body) => {
        resolve(body);
      }));
    } else {
      logger.info('Unprocessed error');
      clientPgPromise.detachDb(logger);
