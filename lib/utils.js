/* eslint-disable no-param-reassign, max-len, prefer-destructuring, no-plusplus */
...
module.exports.executeQuery = async function executeQuery(sql, values, cfg) {
  console.log('Retries left:', cfg.retriesLeft)
  const self = this;
  ...
  return db.tx((t) => {
    ...
    if (error.message === 'deadlock detected' && retriesLeft) {
      new Promise((resolve, reject) => setTimeout(() => {
        console.log('Trying to re-run expression in 1 second');
        cfg.retriesLeft = --retriesLeft;
        clientPgPromise.detachDb();
        self.executeQuery(sql, values, cfg);
      }, 1000),
      ((error, response, body) => {
        resolve(body);
      }));
