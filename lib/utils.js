...
function prepareQuery(sql, body) {
  ...
  const regExpWhiteList = /^(?=.*INSERT INTO|CREATE|DROP|UPDATE|ALTER|DELETE|SELECT|TRUNCATE.*)(?!.*(?:ATTACH|DETACH)).*$/si;
  const result = regExpWhiteList.test(sql);
  if (!result) {
    const errMessage = 'The SQL query contains command from Black list';
